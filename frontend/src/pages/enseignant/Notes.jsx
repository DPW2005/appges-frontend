...
const MES_UES = [
    {
        ...
        etudiants: [
            ...
            { id: 6, nom: 'Alain Biya', note: 11.0 },
        ]
    },
    {
        id: 2, code: 'INFO402', intitule: 'Projet tutoré', niveau: 'L3', semestre: 'S2',
        etudiants: [
            { id: 7, nom: 'Boris Nguema', note: null },
            { id: 8, nom: 'Fatima Oumarou', note: 14.5 },
            { id: 9, nom: 'Patrick Owona', note: null },
            { id: 10, nom: 'Laure Ondoa', note: 13.0 },
        ]
    },
]
...
function Avatar({ nom }) {
    ...
    const colors = ['bg-emerald-500', 'bg-teal-500', 'bg-blue-500', 'bg-violet-500', 'bg-amber-500']
...
function NoteInput({ value, onChange }) {
    return (
        <input
            ...
            placeholder="—"
            className="w-20 text-center text-sm font-semibold text-slate-800 bg-slate-50 border border-slate-200 rounded-lg px-2 py-1.5 focus:outline-none focus:border-emerald-400 focus:ring-1 focus:ring-emerald-100 placeholder:text-slate-300"
        />
    )
}

export default function EnseignantNotes() {
    const [ues, setUes] = useState(MES_UES)
    const [ueActive, setUeActive] = useState(MES_UES[0].id)
        ...
        <div className="p-6 space-y-5">
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-slate-500 mt-1">Dr. Paul Mbarga — Saisie des notes par UE</p>
                </div>
                ...
                {ues.map(u => {
                    ...
                    return (
                        <button key={u.id} onClick={() => setUeActive(u.id)}
                            ...
                            <span className={`text-xs px-1.5 py-0.5 rounded-full font-semibold ${ueActive === u.id ? 'bg-emerald-500 text-white' : 'bg-slate-100 text-slate-500'
                                }`}>{nb}/{tot}</span>
                        </button>
                    )
                })}
